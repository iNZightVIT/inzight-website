...
To load an example data set, go to the __File__ menu and click {{% inzversion desktop="__Example data ...__." lite="__Dataset Examples__." %}} You'll be presented with this window:
...
{{% /inzlite %}}
In the example above, we've selected the "Census at School 500" dataset from the Default iNZight module. We will be using this dataset a lot for many of our examples. Click {{% inzversion desktop="__OK__" lite="__Select set__" %}} to load the data into iNZight. {{% inzversion lite="You can now head to __Visualize__ to start creating graphs." %}}
...
{{% note "If you load a dataset with too many variables and/or rows, iNZight only displays a list of variable names. Click __Dataset__ > __View full dataset__ to get a spreadsheet view in this case." %}}

Now you're ready to create your first graph! The variable names at the top of the spreadsheet view are draggable---go ahead and try dragging __"travel"__ onto the Variable 1 box underneath (labeled "Select/Drag-drop Variable 1"). Congratulations---you've created a **bar chart** of `travel`, which is exactly the best way of graphing this type of ***categorical*** variable!
...
These are variables that have distinct *categories*, *levels*, or *groups*. In the example above, the variable `travel` can take values such as "bike" or "bus". These might also be referred to as **factor** or **qualitative** variables.
{{% /tip %}}
...
{{% inzversion desktop="Another way of " lite="To graph another variable, simply click the dropdown box under **Select first variable:** and choose another variable, for example `height`." %}} 

This produces a **dot plot** in which all of the 500 student heights are "dropped" onto the graph and allowed to stack if the land on top of an existing point. This is a great way of looking a **numeric variables** and how their values are distributed.
{{% inzdesktop %}}
...
These are variables that have values which can be *measured* or *counted*. In the example above, the variable `height` can take any positive value as measured using a ruler. These might also be referred to as **continuous** or **quantitative** variables.
...
1. What is the most popular way students in this sample get their lunch?<br/>
  *Hint: the variable called `getlunch` tells us where each student gets their lunch from.*
2. Some students spend more on their monthly phone bills than others: what was the most a student spends, on average?<br/>
  *Hint: students monthly spends are contained in a variable called `cellcost`.*
